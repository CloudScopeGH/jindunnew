...
public class MyDaoGenerator {
    ...
    public static void main(String args[]) throws Exception {
        ...
        addMessage(schema);
        addPhoneRecord(schema);

        new DaoGenerator().generateAll(schema, "./app/src/main/java-gen");

    }

    private static void addPhoneRecord(Schema schema) {
        Entity phoneRecord = schema.addEntity("PhoneRecord");
        phoneRecord.addIdProperty();

        phoneRecord.addStringProperty("image"); //头像
        phoneRecord.addStringProperty("duration"); //时长
        phoneRecord.addStringProperty("userid"); //id
        phoneRecord.addStringProperty("name");
        phoneRecord.addStringProperty("status"); //状态 呼入，呼出
    }
